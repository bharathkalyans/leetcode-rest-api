...
export default function expressListRoutes(app, opts) {
			...
			if (stack.route) {
				...
				for (const route of stack.route.stack) {
					...
					if (!routeLogged[method] && method) {
						const stackMethod = options.color
							? colorMethod(method)
							: method;
						const stackSpace = spacer(
							options.spacer - method.length
						);
...
export const allRoutes =(app) => expressListRoutes(app).map((x) => {
	return {
		method: x.method,
